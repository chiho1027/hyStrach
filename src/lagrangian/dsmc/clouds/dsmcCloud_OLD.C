...
)
{
  if( thetaVProduct.size() > 0 )
  {
    scalar ChiARemain = ChiA;
    
    forAll(thetaVProduct, m)
    {
      
      label iMaxProduct = ( Ec/(physicoChemical::k.value()*thetaVProduct[m]) );
      if(iMaxProduct == 0)
      {
	vibLevel[m] = 0;
        continue;
      ...
      if(ChiB >= 2.0)
      {
	ChiAM = thetaVProduct[m]/TMacro/(exp(thetaVProduct[m]/TMacro)-1.0);
      }
      else
      {
	ChiAM = 1.0;
      ...
      */
      ...
      scalar ChiAM = ChiA/thetaVProduct.size();//thetaVProduct[m]/TMacro/(exp(thetaVProduct[m]/TMacro)-1.0);
      
      scalar ChiAMinusOne = ChiAM - 1.0;
      ChiARemain -= ChiAM;
 
      //scalar ChiAMinusOne = ChiA - 1.0;
      scalar ChiBMinusOne = ChiB + ChiARemain - 1.0;
      
      /*
      if ( ChiAMinusOne == 0.0 )
      {
	EVib = Ec*( 1.0 - pow(rndGen_.sample01<scalar>(), 1.0/ChiB) );
	iDash = EVib/(physicoChemical::k.value()*thetaVProduct[m]);
      }
      */
      ...
      if ( ChiAMinusOne  == 0.0)
      {
	  ratio = 1.0 - pow(rndGen_.sample01<scalar>(), 1.0/ChiB);
      }
      else if (ChiAMinusOne < SMALL && ChiBMinusOne < SMALL)
      {
	// post-collision quantum number
	
	EVib = Ec*rndGen_.sample01<scalar>();
	iDash = EVib/(physicoChemical::k.value()*thetaVProduct[m]);
      }  
      else
      {
	...
	do // acceptance - rejection
	{
	  ...
	  iDash = randomLabel(0, iMaxProduct);
          
	  EVib = iDash*physicoChemical::k.value()*thetaVProduct[m];
	  
	  // - equation 5.61, Bird
	  func = pow(1.0 - EVib/Ec, ChiBMinusOne);
	...
	*/
	...
	{
	  func = 0.0;
	  ratio = rndGen_.sample01<scalar>();
	  
	  // - equation 5.61, Bird
	  if (ChiAMinusOne < SMALL)
	  {
	    func = pow(1.0 - ratio, ChiBMinusOne);
	  }
	  else if (ChiBMinusOne < SMALL)
	  {
	    func = pow(ratio, ChiAMinusOne);
	  }
	  else
	  {
	    func =
	      pow
	      (
	         (ChiAMinusOne + ChiBMinusOne)*(ratio)/ChiAMinusOne,
	         ChiAMinusOne
	       )
	      *pow
	      (
	         (ChiAMinusOne + ChiBMinusOne)*(1 - ratio)
	         /ChiBMinusOne,
	         ChiBMinusOne
	       );
	  }
      ...
      iDash = ratio*Ec/(physicoChemical::k.value()*thetaVProduct[m]);//randomLabel(0, iMaxProduct);     
      vibLevel[m] = iDash;
      //- Relative collision energy after vibrational energy redistribution
      Ec -= iDash*physicoChemical::k.value()*thetaVProduct[m]; 
    }
  }
}
